...
test('PiAcpAgent: boolean config option only advertised when client supports configOptions.boolean', async () => {
  const realSetTimeout = globalThis.setTimeout
  ;(globalThis as any).setTimeout = () => 0 as any
  ...
  try {
    ...
    const session = {
      ...
      proc: {
        ...
        async getState() {
          return { thinkingLevel: 'medium', model: { provider: 'test', id: 'beta' }, autoCompactionEnabled: false }
        }
      },
      setStartupInfo() {},
      sendStartupInfoIfPending() {}
...
test('PiAcpAgent: setSessionConfigOption maps auto_compaction boolean to pi and emits config_option_update', async () => {
  const conn = new FakeAgentSideConnection()
  const setAutoCompactionCalls: boolean[] = []
  ...
  const session = {
    ...
    proc: {
      ...
      async getState() {
        return { thinkingLevel: 'medium', autoCompactionEnabled: true }
      },
      async setAutoCompaction(enabled: boolean) {
        setAutoCompactionCalls.push(enabled)
      }
    }
  }
  ...
  assert.deepEqual(setAutoCompactionCalls, [false])
  assert.equal(result.configOptions.find(option => option.id === 'auto_compaction')?.currentValue, true)
  assert.equal(conn.updates[0]!.update.sessionUpdate, 'config_option_update')
})

test('PiAcpAgent: setSessionConfigOption rejects boolean value for select options', async () => {
  const conn = new FakeAgentSideConnection()
  ...
  await assert.rejects(
    agent.setSessionConfigOption({ sessionId: 's1', configId: 'model', value: true } as any),
    /Invalid params/
  )
})
